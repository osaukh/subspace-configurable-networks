...
parser.add_argument('--dimensions', default=3, type=int)    # has no effect in One4All
    ...
    for fixed_angle in fixed_angles:
        ...
        def train(dataloader, model, loss_fn, optimizer, fixed_angle):
            for batch, (X, y) in enumerate(tqdm(dataloader, desc='Training')):
                ...
                X = TF.rotate(X, angle=fixed_angle)
        ...
        def validate(dataloader, model, loss_fn, angle):
            model.eval()
        ...
        for t in range(args.epochs):
            print(f"=================\n Epoch: {t + 1} \n=================")
            train(train_loader, model, loss_fn, optimizer, fixed_angle)
            test_acc, test_loss = validate(test_loader, model, loss_fn, fixed_angle)
            wandb.log({"test/loss": test_loss, "test/acc": test_acc})
        print("Done!")

        result_dict[str(fixed_angle)] = test_acc

    ######## write to the bucket
    destination_name = f'{args.output}/rotation/One4One/{save_dir}'
