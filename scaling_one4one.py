...
def main():
    ...
    for fixed_scale in fixed_scales:
        ...
        def train(dataloader, model, loss_fn, optimizer, fixed_scale):
            for batch, (X, y) in enumerate(tqdm(dataloader, desc='Training')):
                X, y = X.to(device), y.to(device)
                X = TF.affine(X, scale=fixed_scale, angle=0, translate=(0, 0), shear=0.0)
        ...
        def validate(dataloader, model, loss_fn, scale):
            model.eval()
        ...
        for t in range(args.epochs):
            print(f"=================\n Epoch: {t + 1} \n=================")
            train(train_loader, model, loss_fn, optimizer, fixed_scale)
            test_acc, test_loss = validate(test_loader, model, loss_fn, fixed_scale)
            wandb.log({"test/loss": test_loss, "test/acc": test_acc})
        print("Done!")

        result_dict[str(fixed_scale)] = test_acc

    ######## write to the bucket
    destination_name = f'{args.output}/scaling/One4One/{save_dir}'
