...
parser.add_argument('--transform', type=str, help="pitchshift | speed | None")
    ...
    def train(model):
        ...
        for X in tqdm(train_loader, "Training"):
            ...
            if args.transform == "pitchshift":
                factors = torch.FloatTensor(1,).uniform_(-10, 10) #for pitchshift
            if args.transform == "speed":
                factors = torch.FloatTensor(1,).uniform_(0.1, 1.) #for speed
            if args.transform == None:
                factors = torch.Tensor([1., ]).to(device) #for None
            data = transform_audio(transform_type= args.transform, waveform=data, sample_rate=16000, factor=factors, device=device)
            output = model(data)
    ...
    def validate(model):
        ...
        with torch.no_grad():
            for X in tqdm(test_loader, "Validating"):
                ...
                # apply transform with random parameter from a range
                if args.transform == "pitchshift":
                    factors = torch.FloatTensor(1,).uniform_(-10, 10).to(device) #for pitchshift
                if args.transform == "speed":
                    factors = torch.FloatTensor(1,).uniform_(0.1, 1).to(device) #for speed
                if args.transform == None:
    ...
    def calculate_estimates(model, data_loader):
        with torch.no_grad():
            for X in tqdm(data_loader):
                ...
                model(data, hyper_x=factors[0].to(device))
