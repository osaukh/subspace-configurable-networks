...
__all__ = ['MLP', 'MLPB', 'HHN_MLP', 'HHN_MLPB', 'mlp', 'mlpb', 'hhn_mlp', 'hhn_mlpb']
...
class MLP(nn.Module):
    def __init__(self, n_layers, n_units, n_channels, num_classes=10):
        ...
        mid_layers.extend([nn.Linear(n_units, num_classes, bias=False)])
...
class MLPB(nn.Module):
    def __init__(self, n_layers, n_units, n_channels, num_classes=10):
        ...
        mid_layers.extend([nn.Linear(n_units, num_classes)])
        self.linear_relu_stack = nn.Sequential(*mid_layers)
...
class HHN_MLP(nn.Module):
    def __init__(self, dimensions, n_layers, n_units, n_channels, num_classes=10):
        ...
        self.weight_list_fc2 = self.create_param_combination_linear(in_features=n_units, out_features=num_classes)
...
def mlpb(**kwargs: Any) -> MLPB:
    model = MLPB(**kwargs)
    return model

def hhn_mlp(**kwargs: Any) -> HHN_MLP:
    model = HHN_MLP(**kwargs)
    return model

def hhn_mlpb(**kwargs: Any) -> HHN_MLPB:
    model = HHN_MLPB(**kwargs)
    return model