...
def main():
    ...
    wandb.init(project="SCN_scaling", entity="ahinea", name="Inverse_%s" % save_dir)
    model.to(device)
    print(model)
    ...
    def train(dataloader, model, loss_fn, optimizer):
        for batch, (X, y) in enumerate(tqdm(dataloader, desc='Training')):
            ...
            X = TF.affine(X, scale=scale, angle=0, translate=(0, 0), shear=0.0)  # scale
            X = TF.affine(X, scale=1 / scale, angle=0, translate=(0, 0), shear=0.0)  # scale back
    ...
    ######## write to the bucket
    destination_name = f'{args.output}/scaling/Inverse/{save_dir}'
